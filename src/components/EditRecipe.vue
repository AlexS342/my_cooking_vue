        ...
        <div class="list w100p bc1">
            <div class="container pd3 w100p">
                ...
                <template v-if="showName">
                    ...
                    <input class="inputName mrx3 w100p" id="title" name="title" type="text" v-model="this.recipe.title">
                </template>
            </div>
        </div>
        ...
        <div class="list w100p bc3">
            <div class="container pd3 w100p">
                ...
                <template v-if="showProduct">
                    ...
                    <template v-for="(product, index) in this.recipe.products">
                        <div class="itemProduct mrt2 w100p">
                            <input class="inputText pdl1 w60p" type="text" v-model.lazy.trim="this.recipe.products[index].name">
                            <input class="inputNum w20p" type="number" v-model.lazy.number.trim="this.recipe.products[index].quantity">
                            <select class="select w20p" v-model.lazy.trim="this.recipe.products[index].units">
        ...
        <div class="list w100p bc4">
            <div class="container pd3 w100p">
                ...
                <template v-if="showProcess">
                    ...
                    <template v-for="(action, index) in this.recipe.actions">
                        <div class="itemProcess mrt2 w100p">
                            <textarea class="textarea w80p" rows="3" v-model.lazy.trim="this.recipe.actions[index].name"></textarea>
                            <div class="timer w20p pdl2">
                                <input class="timerNumber w100p" type="number" v-model.lazy.number.trim="this.recipe.actions[index].quantity">
                                <select class="timerItem w100p mrt1" v-model.lazy.trim="this.recipe.actions[index].units">
                ...
                <template v-if="showFilter">
                    <div class="allFilter">
                        <div class="filterWRP">
                            ...
                            <select class="filterSelectB" id="typeRecipes" v-model.lazy.trim="this.recipe.type">
                        ...
                        <div class="filterWRP">
                            ...
                            <select class="filterSelectB" id="categoryRecipes" v-model.lazy.trim="this.recipe.category">
                        ...
                        <div class="filterWRP">
                            ...
                            <input class="filterInputB" id="portion" type="number" v-model.lazy="this.recipe.portion">
                        </div>
                        <div class="filterWRP">
                            <p class="filterLabel"><label for="time">Время приготовления</label></p>
                            <input class="filterInputL" id="time" type="number" v-model.lazy.number="this.recipe.fullTime">
                            <select class="filterSelectL" v-model.lazy.trim="this.recipe.fullTimeUnix">
...
export default {
    ...
    data() {
        return {
            ...
            //Массивы для выподающих списков
    ...
    created() {
        ...
        if(this.recipe.full_time == null){
            ...
            this.recipe.fullTimeUnix = 'мин.'
        }else{
            let str = this.recipe.full_time
            let arrStr = str.split(' ')
            this.recipe.fullTime = Number(arrStr[0])
            this.recipe.fullTimeUnix = arrStr[1]
        }
    ...
    methods: {
        sendRecipe: async function (){
            ...
            await axios
                .post('/api/edit-my-recipe', {
                    id:this.sendData.id,
                    user_id:this.sendData.user_id,
        ...
        getDataForm: function (){
            ...
            //Удаляем пустые объекты "Продукт"
            let i = 0;
            let arrProduct=[]
            for(product in this.recipe.products){
                if(this.recipe.products[product].name !== null && this.recipe.products[product].name.length > 2){
                    ++i
                    arrProduct.push(this.recipe.products[product])
                }
            }
            this.recipe.products = arrProduct
            ...
            //В поле количество продукта меняем null на пустую строку
            for(product in this.recipe.products){
                if(this.recipe.products[product].quantity == null){
                    this.recipe.products[product].quantity = ''
                }
            }

            //Удаляем пустые объекты "Действие"
            i = 0
            let arrAction=[]
            for(action in this.recipe.actions){
                if(this.recipe.actions[action].name !== null && this.recipe.actions[action].name.length > 2){
                    ++i
                    arrAction.push(this.recipe.actions[action])
                }
            }
            this.recipe.actions = arrAction
            if(i < 2){
                this.actionErr = true
            ...
            //В поле количество продукта меняем null на пустую строку
            // console.log(this.recipe.actions)
            for(action in this.recipe.actions){
                if(this.recipe.actions[action].quantity == null){
                    this.recipe.actions[action].quantity = ''
                }
            }
            ...
            this.sendData = {
                ...
                actions:this.recipe.actions,
                type:this.recipe.type,
                category:this.recipe.category,
                portion:this.recipe.portion,
                full_time:this.recipe.fullTime + ' ' + this.recipe.fullTimeUnix
            }
        ...
        addInputProduct: function () {
            // let size = Object.keys(this.recipe.products).length;
            // ++size
            // let itemName = size
            // this.recipe.products[itemName] = {name:"", quantity:"", units:"г.",}
            this.recipe.products.push({name:"", quantity:"", units:"г.",})
        ...
        addInputAction: function () {
            ...
            this.recipe.actions.push({name:"", quantity:"", units:"мин."})
            // console.log(this.recipe.actions)
        },
        ...
        validationForm: function (){
            ...
            if(this.recipe.title.length < 5){
                ...
                return false
            }else if(this.recipe.title.length > 150){
                this.titleErr = true
            ...
            for(product in this.recipe.products){
                ...
                if(this.recipe.products[product].name.length !== 0){
                    if(this.recipe.products[product].name.length < 3){
                        this.productErr = true
                        this.massageErr = 'Короткое название одного из продуктов. Минимум 3 символа.'
                        break
                    }else if (this.recipe.products[product].name.length > 100){
                        this.productErr = true
                ...
                if(this.recipe.products[product].quantity.length !== 0 || this.recipe.products[product].quantity === 0){
                    this.recipe.products[product].quantity = Number(this.recipe.products[product].quantity)
                    if(this.recipe.products[product].quantity < 1){
                        this.productErr = true
                        this.massageErr = 'Маленький вес одного из продуктов. Минимум 1.'
                        break
                    }else if(this.recipe.products[product].quantity > 1000){
                        this.productErr = true
                ...
                if(this.recipe.products[product].quantity.length === 0 && this.recipe.products[product].units !== 'по вкусу'){
                    this.productErr = true
            ...
            for(action in this.recipe.actions){
                ...
                if(this.recipe.actions[action].name.length !== 0){
                    if(this.recipe.actions[action].name.length < 5){
                        // console.log(this.recipe.actions[action].name.length)
                        this.actionErr = true
                        this.massageErr = 'Короткое описание одного из процессовв. Минимум 5 символа.'
                        break
                    }
                    else if(this.recipe.actions[action].name.length > 200){
                        this.actionErr = true
                ...
                if(this.recipe.actions[action].quantity.length !== 0){
                    this.recipe.actions[action].quantity = Number(this.recipe.actions[action].quantity)
                    if(this.recipe.actions[action].quantity < 1){
                        this.actionErr = true
                        this.massageErr = 'Указано маленькое время приготовления. Минимум 1'
                        break
                    } else if (this.recipe.actions[action].quantity > 1000){
                        this.actionErr = true
            ...
            else if(typeof this.recipe.fullTime === 'string' && this.recipe.fullTime.length === 0){
                this.recipe.fullTimeUnix = ''
            }
